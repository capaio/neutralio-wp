...
class tds_util {
		...
		if ( null !== $subscriptions ) {
			...
			if ( count( $subscriptions ) ) {
				...
				foreach ( $subscriptions as &$subscription ) {
					...
					if (
                        ...
                        !empty($subscription['is_with_credits'])
                    ) {
						continue;
					}
								...
								if ( false !== $update_result && !$canceled ) {
									...
									if ( false !== $insert_result ) {
										...
										if ( $subscription['payment_type'] === 'stripe' && !empty($stripe_subscription_id) ) {
                                            ...
                                            /*
                                             * in some cases the stripe_subscription_id on $subscription may hold the id of the invoice generated on a stripe payment, so we need to check this is actually a stripe subscription id
                                             * for example this is how a stripe subscription id should look like: `sub_1OhsOUEhF5ct7bo6Eo0o6FIP`
                                             * and this is a stripe invoice id: `in_1OwmOREhF5ab7bo643Wv0oFD`
                                            */
                                            if ( strpos( $stripe_subscription_id, 'sub_' ) !== false ) {
                                                ...
                                                if ( null !== $tds_payment_stripe_results ) {
                                                    ...
                                                    try {
                                                        ...
                                                        $stripe_subscription_billing_cycle_update_result = $stripe_client->subscriptions->update(
                                                            $stripe_subscription_id,
                                                            array(
                                                                'billing_cycle_anchor' => 'now',
                                                                'proration_behavior' => 'none',
                                                                //'metadata' => [ 'billing_cycle_anchor' => $wpdb->insert_id ]
                                                            )
                                                        );
                                                        ...
                                                        td_log::log( __FILE__, __FUNCTION__,
                                                            ...
                                                            array(
                                                                ...
                                                                'stripe_subscription_billing_cycle_update_result' => $stripe_subscription_billing_cycle_update_result
                                                            )
                                                        );
                    ...
                    if( $subscription['is_unlimited'] || $subscription['is_with_credits'] ) {
			...
			if ( null !== $subscriptions && count( $subscriptions ) ) {
				...
				foreach ( $subscriptions as &$subscription ) {
                    ...
                    if( !$subscription['is_free'] && ( !$subscription['is_unlimited'] || ( $subscription['status'] === 'trial' && intval($subscription['trial_days']) > 0 ) ) && !$subscription['is_with_credits'] ) {
                    ...
                    $subscription['formatted_cycle_interval'] = '-';
                    if( !$subscription['is_free'] && !$subscription['is_unlimited'] && !$subscription['is_with_credits'] ) {
                        $subscription_ci_format = tds_util::ci_format( $subscription['cycle_interval'], $subscription['cycle_interval_count'] );
                        $subscription['formatted_cycle_interval'] = $subscription['cycle_interval_count'] . ' ' . $subscription_ci_format;
                    }
			...
			if ( null !== $subscriptions && count( $subscriptions ) ) {
				...
				foreach ( $subscriptions as &$subscription ) {
                    ...
                    if( $subscription_user_data && in_array( 'administrator', $subscription_user_data->roles ) ) {
                        continue;
                    }
    ...
    static function get_account_page_url() {
    ...
    static function stripe_pm_format( $payment_method_obj ) {
        ...
        switch ($payment_method_type) {
            ...
            case 'card':
                ...
                $payment_method_output .= '<span class="tds-s-list-text-sep"> | </span>';
                $payment_method_output .= __td('Expires', TD_THEME_NAME) . ' ' . $card_exp_month . '/' . $card_exp_year;
    ...
    static function ci_format( $cycle_interval, $cycle_interval_count = 1 ) {
        ...
        return __td(ucfirst($cycle_interval_output), TD_THEME_NAME);
    ...
    static function create_subscription( $subscription_data ) {
        ...
        if ( $valid_plan ) {
            ...
            $data_values = array(
                ...
                'is_with_credits' => $is_with_credits,
                'credits' => $credits,
                'start_date' => date('Y-m-d'),
            );
            $data_format = array(
    ...
    /*
     * internal utility function to get available credits for a user
     */
    static function get_user_available_credits( $user_id ) {

        $available_credits = 0;

        $subscriptions_query = "
            SELECT tds_subscriptions.*
            FROM tds_subscriptions 
            WHERE tds_subscriptions.user_id = %s
            AND tds_subscriptions.status = 'active'
            AND tds_subscriptions.is_with_credits = 1
        ";

        global $wpdb;

        // get all active user subscriptions with credits
        $subscriptions_results = $wpdb->get_results( $wpdb->prepare( $subscriptions_query, $user_id ), ARRAY_A );
        if ( null !== $subscriptions_results && count($subscriptions_results) ) {
            foreach( $subscriptions_results as $subscription ) {
                $subscription_credits = !empty($subscription['credits']) ? intval($subscription['credits']) : 0;
                if ( $subscription_credits ) {
                    $available_credits += $subscription_credits;
                }
            }
        }

        // if there are no available credits at this point... return
        if ( $available_credits === 0 ) {
            return $available_credits;
        }

        // get user credits locker meta
        $tds_user_locker_credits = get_user_meta( $user_id, 'tds_locker_credits', true );

        // calculate credits user used to unlock posts
        $tds_used_user_credits = 0;
        if ( is_array($tds_user_locker_credits) ) {
            foreach ( $tds_user_locker_credits as $post_id => $post_used_credits ) {
                $credits = intval($post_used_credits);
                if ( $credits > 0 ) {
                    $tds_used_user_credits += $credits;
                }
            }
        }

        // if user has used credits ...
        if ( $tds_used_user_credits > 0 ) {

            // if user has a greater number of credits used than available credits ... not ok !!! ... return 0 for now @todo treat this case
            if ( $tds_used_user_credits > $available_credits ) {
                return 0;
            } else {
                // ... subtract used credits from available credits
                $available_credits = $available_credits - $tds_used_user_credits;
            }
    ...
    /*
     * internal utility function to get billing details for a user
     */
    static function get_user_billing_details( $user ) {
        ...
        // custom user meta
        $user_first_name = get_user_meta( $_user->ID, 'first_name', true );
        $user_last_name = get_user_meta( $_user->ID, 'last_name', true );

        // user data
        $user_email = $_user->user_email;

        // user billing details array init
        $user_billing_details = [
            'billing_first_name' => empty($user_first_name) && empty($user_last_name) ? $user_email : $user_first_name,
        ...
        $billing_details_results = $wpdb->get_results(
            $wpdb->prepare( "SELECT * FROM tds_billing WHERE user_id = %s", $_user->ID ),
            ARRAY_A
        );

        // user db billing details exist flag init
        $user_billing_details_exist = false;

        // process db user billing details if found
        if ( $billing_details_results !== null && count($billing_details_results) ) {

            foreach ( $billing_details_results as $user_billing_data ) {

                foreach ( $user_billing_data as $key => $value ) {

                    if ( $key === 'billing_country' && !empty($value) ) {
                        $user_billing_details[$key] = tds_util::get_country_name($value);
                    } else {
                        $user_billing_details[$key] = $value;
                    }
            ...
            // update user db billing details exist flag
            $user_billing_details_exist = true;

        }

        // set user db billing details exist flag
        $user_billing_details['billing_details_exist'] = $user_billing_details_exist;
    ...
    /*
     * internal utility function to get billing details from a subscription
     */
    static function get_subscription_billing_details( $tds_subscription_data ) {

        if ( empty($tds_subscription_data) ) {
            return null;
        }

        // billing details array init
        $billing_details = [
            'billing_first_name' => '',
            'billing_last_name' => '',
            'billing_company_name' => '',
        ...
        // update billing_details array with billing details values from subscription's data array
        foreach ( $billing_details as $key => &$value ) {
            $value = $tds_subscription_data[$key];
        }
    ...
    /*
     * internal utility function to get details for a payment type
     */
    static function get_payment_details( $payment_type ) {
        ...
        switch ( $payment_type ) {
            ...
            case 'direct':
                ...
                $payment_details['label'] = __td('Direct Bank Transfer', TD_THEME_NAME );

                $tds_payment_bank_results = $wpdb->get_results("SELECT * FROM tds_payment_bank LIMIT 1", ARRAY_A );
                if ( null !== $tds_payment_bank_results && count($tds_payment_bank_results) ) {

                    $payment_bank_details = $tds_payment_bank_results[0];

                    if ( $payment_bank_details['is_active'] ) {
                        $payment_details['direct_payment_bank'] = $payment_bank_details['bank_name'];
                        $payment_details['direct_payment_account_name'] = $payment_bank_details['account_name'];
                        $payment_details['direct_payment_account_number'] = $payment_bank_details['account_number'];
                        $payment_details['direct_payment_routing_number'] = $payment_bank_details['routing_number'];
                        $payment_details['direct_payment_iban'] = $payment_bank_details['iban'];
                        $payment_details['direct_payment_bic_swift'] = $payment_bank_details['bic_swift'];
                        $payment_details['direct_payment_instruction'] = $payment_bank_details['instruction'];
                    }
            ...
            case 'paypal':
                $payment_details['label'] = __td('PayPal', TD_THEME_NAME );
                break;

            case 'stripe':
                $payment_details['label'] = __td('Stripe', TD_THEME_NAME );
                break;

            default:
                $payment_details['label'] = __td('-', TD_THEME_NAME );
                break;
    ...
    /*
     * internal utility function to format subscription statuses
     */
    static function subscription_status( $subscription_status, $canceled = false, $formatted = true ) {

        $status_style = '';

        switch ( $subscription_status ) {
            case 'free':
                $status_class = 'tds-s-table-status-free';
                $subscription_status = __td('Free', TD_THEME_NAME );
                break;
            case 'active':
                $status_style = ' style="background-color: #E2F3DF; color: #317A25;"';
                $status_class = 'tds-s-table-status-active';
                $subscription_status = __td('Active', TD_THEME_NAME );
                break;
            case 'trial':
                $status_class = 'tds-s-table-status-trial';
                $subscription_status = __td('Trial', TD_THEME_NAME );
                break;
            case 'blocked':
                $status_class = 'tds-s-table-status-generic';
                $subscription_status = __td('Blocked', TD_THEME_NAME );
                break;
            case 'closed':
                $status_class = 'tds-s-table-status-canceled';
                if ( $canceled ) {
                    $subscription_status = __td('Canceled', TD_THEME_NAME );
                } else {
                    $subscription_status = __td('Closed', TD_THEME_NAME );
                }
                break;
            case 'closed_not_paid':
                $status_class = 'tds-s-table-status-not-paid';
                $subscription_status = __td('Not paid', TD_THEME_NAME );
                break;
            case 'paid_incomplete':
                $status_style = ' style="background-color: #FCE8E8; color: #FF0000;"';
                $status_class = 'tds-s-table-status-not-paid';
                $subscription_status = __td('Incomplete payment', TD_THEME_NAME );
                break;
            case 'waiting_payment':
                $status_style = ' style="background-color: #E5F3FF; color: #152BF7;"';
                $status_class = 'tds-s-table-status-waiting';
                $subscription_status = __td('Awaiting payment', TD_THEME_NAME );
                break;
            default:
                $status_class = 'tds-s-table-status-generic';
                $subscription_status = empty($subscription_status) ? __td('N/A', TD_THEME_NAME ) : $subscription_status;
                break;
        }

        if ( $formatted ) {

            return '<div class="tds-s-table-status ' . $status_class . '"' . $status_style . '>' . $subscription_status . '</div>';
