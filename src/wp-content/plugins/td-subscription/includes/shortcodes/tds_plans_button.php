            ...
            $buffy .= '<div class="tds-block-inner td-fix-index">';
				...
				if ( empty($payment_page_id) || !get_post($payment_page_id) instanceof WP_Post ) {
					...
					if ( !empty($data_url_free) ) {
                        ...
                        $auto_unlock = $this->get_att('auto_unlock');

                        global $post;
                        //echo '<pre class="td-container">';
                            //echo '$auto_unlock: ' . print_r($auto_unlock, true) . '<br>';
						...
						if ( !empty($year_plan) ) {
							...
							} elseif ( $auto_unlock && $post instanceof WP_Post ) {
                                $url_year = add_query_arg( 'tds_auto_unlock', $post->ID, $url_year );
                            }
						...
						if ( !empty($month_plan) ) {
							...
							if ( !empty($_REQUEST['ref_url']) ) {
                                ...
                                $url_month = add_query_arg( 'tds_auto_unlock', $post->ID, $url_month );
                            }
