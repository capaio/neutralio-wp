...
class tds_email_locker {
	...
	public function locker_init() {
		...
		if ( $tds_lock_content ) {
            ...
            // check credits locker
            $tds_locker_credits_unlock = !empty($tds_locker_types['tds_locker_credits_unlock']) ? $tds_locker_types['tds_locker_credits_unlock'] : '';

            // if credits unlock is active for this locker
            if ( $tds_locker_credits_unlock && is_user_logged_in() ) {

                // get user locker credits meta
                $tds_user_locker_credits = get_user_meta( get_current_user_id(), 'tds_locker_credits', true );

                // check if current post has been unlocked by current user ( the current post id is found in user's tds_locker_credits meta )
                if ( is_array($tds_user_locker_credits) && array_key_exists( $post->ID, $tds_user_locker_credits ) ) {
                    return;
                }
	...
	public function lock_content($content) {
        ...
        // set locker preview att to false
        $locker_shortcode_atts['tds_locker_preview'] = false;

        // maybe add credits locker
        $tds_locker_credits_unlock = !empty($locker_shortcode_atts['tds_locker_credits_unlock']) ? $locker_shortcode_atts['tds_locker_credits_unlock'] : '';
        $tds_locker_credits = '';
        if ( $tds_locker_credits_unlock ) {
            $tds_locker_credits .= td_global_blocks::get_instance('tds_locker_credits')->render( $locker_shortcode_atts );
        }

        // locker custom page option check, this loads a specific page content instead of tds_locker & tds_locker_credits
        $tds_locker_custom_page_id = !empty($locker_shortcode_atts['tds_locker_custom_page_id']) ? $locker_shortcode_atts['tds_locker_custom_page_id'] : '';
        $tds_locker_custom_page_content = '';
        if ( !empty($tds_locker_custom_page_id) ) {

            $tds_payable = $locker_shortcode_atts['tds_payable'] ?? '';
            //echo '<pre class="td-container">tds_payable: ' . print_r( $tds_payable, true ) . '</pre>';

            $locked = true;
            if ( $tds_payable && is_user_logged_in() ) {

                $tds_paid_subs_page_ids = !empty($tds_locker_types['tds_paid_subs_plan_ids']) ? $tds_locker_types['tds_paid_subs_plan_ids'] : [];
                //echo '<pre class="td-container">$tds_paid_subs_page_ids: ' . var_export( $tds_paid_subs_page_ids, true ) . '</pre>';

                // check subscription plan & status
                if ( $tds_paid_subs_page_ids ) {

                    $user_subscriptions = tds_util::get_subscriptions( get_current_user_id() );
                    $subscriptions = !empty($user_subscriptions['subscriptions']) ? $user_subscriptions['subscriptions'] : [];

                    foreach ( $subscriptions as $subscription ) {

                        // check subscription plan & status
                        if ( in_array( $subscription['plan_id'], $tds_paid_subs_page_ids ) && in_array( $subscription['status'], [ 'free', 'active', 'trial' ] ) ) {
                            $locked = false;
                        }
            ...
            if ( $locked ) {
                ...
                // get page
                $tds_locker_custom_page = get_post($tds_locker_custom_page_id);

                // check page is WP_Post
                if ( $tds_locker_custom_page instanceof WP_Post ) {
                    $tds_locker_custom_page_content = $tds_locker_custom_page->post_content;
                }
        ...
        // tds_locker shortcode render
        $tds_locker = td_global_blocks::get_instance('tds_locker')->render($locker_shortcode_atts);
		...
		switch ( tds_email_locker::instance()->get_content_lock_type() ) {
			...
			case 'partial':
				...
				if ( preg_match( '/\[tds_partial_locker (.*?)]/', $content, $matches ) ) {
                    ...
                    // if custom page
                    if ( !empty($tds_locker_custom_page_content) ) {

                        // return content before locker + tds locker custom page content + locked content (content after locker)
                        return $content_before . $tds_locker_custom_page_content . $tds_locked_content;

                    } else {

                        // return content before locker + tds locker + tds credits locker(if set) + locked content (content after locker)
                        return $content_before . $tds_locker . $tds_locker_credits . $tds_locked_content;
			...
			case 'content':
				...
				if ( preg_match( '/' . $pattern . '/s', $content, $matches ) ) {
                    ...
                    if ( !empty($tds_locker_custom_page_content) ) {
                        ...
                        // return content before locker + tds locker custom page content + locked content (content after locker) + content after
                        return $content_before . $tds_locker_custom_page_content . $tds_locked_content . $content_after;

                    } else {

                        // return content before locker + tds locker + tds credits locker(if set) + locked content (content after locker) + content after
                        return $content_before . $tds_locker . $tds_locker_credits . $tds_locked_content . $content_after;
			...
			default: // full as default
                ...
                if ( !empty($tds_locker_custom_page_content) ) {
                    ...
                    // return tds locker custom page content + locked content (content after locker)
                    return $tds_locker_custom_page_content . $content;

                } else {

                    // return tds locker + tds credits locker(if set) + full locked content
                    return $tds_locker . $tds_locker_credits . $content;
